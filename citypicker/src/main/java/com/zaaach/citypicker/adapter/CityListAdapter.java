...
public class CityListAdapter extends RecyclerView.Adapter<CityListAdapter.BaseViewHolder> {
    ...
    public void scrollToSection(String index) {
        ...
        for (int i = 0; i < size; i++) {
            if (TextUtils.equals(index.substring(0, 1), mData.get(i).getSection().substring(0, 1)) || (index.substring(0, 1).equals("定") && mData.get(i).getSection().contains("自动定位"))) {
    ...
    public BaseViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case VIEW_TYPE_LOCATION:
                view = LayoutInflater.from(mContext).inflate(R.layout.cp_list_item_location_layout2, parent, false);
    ...
    public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
        ...
        if (holder instanceof LocationViewHolder) {
            ...
            ((LocationViewHolder) holder).layRoot_location.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
...
*/
    ...
    public static class LocationViewHolder extends BaseViewHolder {
        ...
        TextView current;
        LinearLayout layRoot_location;

        LocationViewHolder(View itemView) {
            super(itemView);
//            container = itemView.findViewById(R.id.cp_list_item_location_layout);
            current = itemView.findViewById(R.id.tv_location);
            layRoot_location = itemView.findViewById(R.id.lay_root_location);
        }
    }
}
